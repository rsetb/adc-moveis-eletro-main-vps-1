...
export default function ProductForm({ productToEdit, onFinished }: ProductFormProps) {
  ...
  async function onSubmit(values: ProductFormValues) {
    ...
    try {
      if (productToEdit) {
        await updateProduct({ ...productToEdit, ...productData, promotionEndDate: values.promotionEndDate?.toISOString() } as any, logAction, user);
      } else {
        await addProduct({ ...productData, promotionEndDate: values.promotionEndDate?.toISOString() } as any, logAction, user);
      }
      onFinished();
    } catch (error) {
      toast({
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="Ex: A: 20cm x L: 30cm x P: 10cm" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
