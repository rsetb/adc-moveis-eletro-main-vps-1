...
import { cn, toIsoNoon } from '@/lib/utils';
...
export default function CreateOrderPage() {
  ...
  const sellers = useMemo(() => {
    return users
      .filter(u => u.role === 'vendedor' || u.role === 'admin' || u.role === 'gerente' || u.role === 'vendedor_externo' || u.role === 'vendedor_cobranca')
  ...
  const filteredCustomers = useMemo(() => {
    ...
    // Filter by seller if role is vendedor_cobranca
    ...
    // Se não houver busca, limita a 50 para performance
    if (!q) return result.slice(0, 50);

    const filtered = result.filter((c) => {
      const haystack = normalizeText(`${c.code || ''} ${c.name || ''} ${c.cpf || ''} ${c.phone || ''}`);
      return haystack.includes(q);
    });

    // Limita resultados da busca também para evitar travamento se a query for muito genérica
    return filtered.slice(0, 100);
  }, [allCustomers, customerSearch, user]);
  ...
  useEffect(() => {
    ...
    const defaultSellerId = sellers.find(s => s.id === user?.id)?.id || sellers[0]?.id || '';
    if (!currentSellerId || !sellers.some(s => s.id === currentSellerId)) {
      if (defaultSellerId) {
        form.setValue('sellerId', defaultSellerId, { shouldValidate: true });
      }
    }

    const currentFirstDueDate = form.getValues('firstDueDate');
    if (!currentFirstDueDate || !isValid(currentFirstDueDate)) {
      form.setValue('firstDueDate', addMonths(new Date(), 1), { shouldValidate: true });
    }

    didInitDefaultsRef.current = true;
  }, [form, sellers, user?.id]);
  ...
  const installmentPreview = useMemo(() => {
    ...
    for (let i = 0; i < installmentsCount; i++) {
      ...
      newInstallmentDetails.push({
        ...
        dueDate: toIsoNoon(addMonths(firstDueDate, i)),
      });
    }
  ...
  async function onSubmit(values: CreateOrderFormValues) {
    ...
    const normalizedFirstDueDate = new Date(values.firstDueDate);
    normalizedFirstDueDate.setHours(12, 0, 0, 0);

    const installmentDetails = Array.from({ length: values.installments }, (_, i) => ({
      id: `inst-manual-${Date.now().toString().slice(-6)}-${i}`,
      installmentNumber: i + 1,
      amount: installmentValue,
      dueDate: toIsoNoon(addMonths(normalizedFirstDueDate, i)),
    ...
    try {
      ...
      if (savedOrder) {
        toast({
            ...
            description: `O pedido #${savedOrder.id} foi registrado.`,
            variant: "default",
                    ...
                    <Select
                      ...
                      onValueChange={(selectedCustomerId) => {
                        ...
                        if (selectedCustomer?.sellerId) {
                          form.setValue("sellerId", selectedCustomer.sellerId, { shouldValidate: true });
                        }
                      }}
                    >
                      <FormControl>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex flex-col">
                    ...
                    <Select onValueChange={field.onChange} value={field.value} disabled={user?.role === 'vendedor_cobranca'}>
                      <FormControl>
                        <SelectTrigger>
