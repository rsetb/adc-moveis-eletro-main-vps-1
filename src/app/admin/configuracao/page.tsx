...
const settingsSchema = z.object({
  ...
  solicitationsTrashRetentionDays: z.coerce.number().int().min(1).max(365).optional(),
});
...
export default function ConfiguracaoPage() {
  ...
  useEffect(() => {
    if (!settingsLoading && settings) {
      form.reset({
        ...
        solicitationsTrashRetentionDays: settings.solicitationsTrashRetentionDays || 30,
      });
    }
  ...
  const applyBackupData = async (data: any) => {
    ...
    if (data.customerCodeCounter) {
      const raw = data.customerCodeCounter;
      const lastNumber = typeof raw === 'number' ? raw : Number(raw?.lastNumber || 0);
      await updateCustomerCodeCounterAction(lastNumber);
    }
  };
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <FormField
                  control={form.control}
                  name="solicitationsTrashRetentionDays"
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Retenção da Lixeira de Solicitações (dias)</FormLabel>
                      <FormControl>
                        <Input
                          type="number"
                          min={1}
                      ...
                      <FormDescription>
                        Solicitações rejeitadas podem ser restauradas dentro desse período. Padrão: 30 dias.
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
            ...
            {localPermissions ? (
              <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-5 gap-8">
                  ...
                  <div>
                    <h3 className="font-semibold mb-4 capitalize">Vendedor Cobrança</h3>
                    <div className="space-y-3">
                      {ALL_SECTIONS.map(section => (
                        <div key={`vendedor_cobranca-${section.id}`} className="flex items-center space-x-2">
                          <Checkbox
                            id={`vendedor_cobranca-${section.id}`}
                            checked={localPermissions.vendedor_cobranca?.includes(section.id)}
                            onCheckedChange={(checked) => handlePermissionChange('vendedor_cobranca', section.id, !!checked)}
                          />
                          <label
                            htmlFor={`vendedor_cobranca-${section.id}`}
                            className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70"
                          >
                            {section.label}
                          </label>
