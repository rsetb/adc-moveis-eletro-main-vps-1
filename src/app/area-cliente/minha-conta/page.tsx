  ...
  const customerFinancials = useMemo(() => {
    ...
    const totalPago = sortedCustomerOrders.reduce((sum, order) => {
      if (order.paymentMethod === 'Crediário') {
        ...
        return sum + (order.downPayment || 0) + paid;
      }
    ...
    const saldoDevedor = Math.max(0, totalComprado - totalPago);
        ...
        <div className="container mx-auto py-12 px-4">
            <header className="mb-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-muted-foreground">Olá, {(customer.name || '').split(' ')[0] || customer.name}! Bem-vindo(a) de volta.</p>
                </div>
