...
export default function ProductDetailPage() {
  ...
  const similarProducts = useMemo(() => {
    ...
    const base = products.filter(p => !p.isHidden && p.id !== product.id);
    const bySub = base.filter(p => p.subcategory && product.subcategory && p.subcategory.toLowerCase() === product.subcategory.toLowerCase());
    const byCat = base.filter(p => p.category && p.category.toLowerCase() === product.category?.toLowerCase());
    const list = (bySub.length > 0 ? bySub : byCat)
      ...
      <div className="container mx-auto max-w-6xl py-12 px-4">
        ...
        <div className="grid md:grid-cols-2 gap-8 lg:gap-12">
          ...
          <div className="flex flex-col">
            ...
            {product.dimensions && <p className="text-sm text-muted-foreground mt-1">Dimensões: {product.dimensions}</p>}
        ...
        {similarProducts.length > 0 && (
          <div className="mt-12">
            <h2 className="text-xl font-bold mb-4">Produtos similares</h2>
